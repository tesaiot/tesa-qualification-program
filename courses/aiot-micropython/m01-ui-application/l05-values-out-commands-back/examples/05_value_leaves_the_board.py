...
# ดูอีกฝั่ง : เปิด shared/web/my_first_reader.html ในเบราว์เซอร์ แก้ TEAM ให้ตรงกับไฟล์นี้
...
if not 4 <= len(TEAM) <= 16 or TEAM == "team00" or TEAM.strip("abcdefghijklmnopqrstuvwxyz0123456789"):
    stop_here(st_wifi, "1) WiFi      ยังไม่ได้ตั้งชื่อทีม",
              "แก้ TEAM เป็นรหัส a-z 0-9 ยาว 4-16 ตัว เช่น nok4821")
...
# เปิดหน้าเว็บอ่านค่า ต่อท้ายลิงก์ด้วยรหัสเดียวกับ TEAM (หน้าบนเว็บ AIC รับแค่รูป teamNN ถ้าไม่ใช่ให้เปิดไฟล์ใน shared/web) เช่น
...
# จากนั้นตั้ง TEAM ของบอร์ดสองตัว (หรือบอร์ดกับ Emulator) ให้ชนกันชั่วคราว แล้วรันพร้อมกัน
