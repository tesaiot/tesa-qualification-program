...
| สองบอร์ดผลัดกันหลุดทั้งบทเรียน | `client_id` ซ้ำกัน broker เตะตัวเก่าออกทุกครั้งที่ตัวใหม่เข้ามา บน broker สาธารณะชนได้กับทุกคนในโลก | ใช้ `TEAM` ที่ไม่ซ้ำใคร (เช่น `nok4821` หรือเลขทีมที่ผู้จัดแจก) และคง `"bento-aiot-"` ไว้หน้า `DEVICE_ID` |
...
**งานหลักของทีมคือไฟล์ตัวอย่างของชุดบทเรียนนี้เอง** ทำตามลำดับในตารางแล้วทำบล็อก "ตาคุณ" ท้ายไฟล์ให้ครบทุกไฟล์ · ผลที่ทีมจดได้ เอาไปเติมในไฟล์ฝึก [`s02_ai_observer.py`](https://github.com/tesaiot/tesa-qualification-program/blob/main/courses/aiot-micropython/m01-ui-application/l06-link-lab/practice/s02_ai_observer.py) ซึ่งเป็นสมุดบันทึกผลการทดลองของทีม (เฉลยอยู่ที่ `solution/` ของบทเรียนนี้)
...
<div style="font-size:.56em;color:#90a4ae">**04** ให้บอร์ดฟังคลื่นทั้งห้อง แล้วบอกว่าใครอยู่ตรงไหนบ้าง · **05** ค่าที่วัดได้บนโต๊ะนี้ ไปโผล่บนเครื่องคนอื่น (ในภาพตั้ง `TEAM = "nok4821"` หัวข้อจึงเป็น `bento-aiot/nok4821/telemetry` บน `broker.hivemq.com`) · **06** คนอื่นพิมพ์คำสั่งจากที่ไกล แล้วไฟบนโต๊ะเราติด (ในภาพฟัง `bento-aiot/nok4821/cmd`)</div>
...
<div style="font-size:.56em;color:#90a4ae">**07** บอร์ดจำได้เองว่าจะต่อไปที่ไหน แม้ถอดไฟแล้วเสียบใหม่ (ไฟล์อ่านอย่างเดียว และจบหลังแสดงคลังค่าตั้งถ้ายังไม่ใส่ `PLATFORM_BROKER` อย่างที่เห็นในภาพ)</div>
