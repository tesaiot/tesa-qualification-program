...
| **How** | ประกอบยังไงให้ใช้งานได้จริง | กรอก canvas ห้าช่อง Sense → Decide → Show → Send → Act ให้ครบก่อนแตะโค้ด แล้วเติมโครง [`s12_capstone_starter.py`](https://github.com/tesaiot/tesa-qualification-program/blob/main/courses/aiot-micropython/m05-capstone/l03-build-and-present/practice/s12_capstone_starter.py) ที่จุด "ทีมเขียนเอง" ห้าจุด | หกไฟล์ตัวอย่าง + โครงตั้งต้น + นำเสนอ 10 นาที |
...
|---|---|---|---|
| `lcd` | **4** — `print` `console` `clear` `theme` | บทเรียน 1.1–1.3 | Show — ทางที่ง่ายที่สุดตอนยังไม่มีหน้า HMI |
| `gpio` | **18** — ระดับโมดูล 7 · เมธอดของ `LED` 8 · ของ `Button` 3 | บทเรียน 2.1–2.3 | **Act** — ทางเดียวในคอร์สนี้ที่สั่งของจริงให้ขยับ |
| `ui` | **52** บนโมดูล · บวกเมธอดของ `Widget` อีก **14** | บทเรียน 2.4–2.6 · 7 · 8 | Show — และเป็นทางที่คนสั่งงานเข้ามาด้วย |
