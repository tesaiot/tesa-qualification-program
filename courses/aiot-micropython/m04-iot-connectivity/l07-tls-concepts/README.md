...
objectives:
  ...
  - {th: อธิบายว่าพอร์ต 8884 ถูกเลือกจาก tls_mode ไม่ใช่คีย์ port และทำไม MQTTs จากบอร์ดเข้า TESAIoT CE ที่ทีมติดตั้งเองไม่ผ่าน, en: 'Explain that port 8884 is chosen by tls_mode rather than the port key, and why MQTTs from the board cannot reach a self-installed TESAIoT CE.'}
...
3. อธิบายว่าพอร์ต 8884 ถูกเลือกจาก tls_mode ไม่ใช่คีย์ port และทำไม MQTTs จากบอร์ดเข้า TESAIoT CE ที่ทีมติดตั้งเองไม่ผ่าน
...
ก่อนจบบทเรียนนี้ ทุกทีมต้องได้ค่าประจำตัวสี่ค่าจากผู้สอน: `device_id` · `api_key` · `mqtt_pass` · ชื่อโฮสต์ของ broker
จดลงบันทึกการเรียนก่อนแตะโค้ดในบทเรียน 4.8

- **อุปกรณ์:** บอร์ด Eva Kit หรือ TESAIoT Dev Kit ที่ลงเฟิร์มแวร์ MicroPython ของ BENTO แล้ว หรือ BENTO Emulator ใน [BENTO IDE](https://ide.tesaiot.dev/) (บทเรียนนี้ไม่มีโค้ดให้รัน การต่อ TLS จริงเริ่มในบทเรียน 4.8 และต้องใช้บอร์ดจริงที่ได้ตัวตนจากผู้สอนแล้ว)
...
ทั้ง `username: team03` รหัสผ่านของทีม และ JSON ของค่าเซนเซอร์ บนพอร์ต 8884 เห็นแค่ TLS Application Data เป็นไบต์ที่อ่านไม่ออก
...
MQTTs จากบอร์ดจึงเข้า CE ที่ทีมติดตั้งเองไม่ผ่าน เพราะสคริปต์ติดตั้งของ CE สุ่ม root CA ใหม่ทุกครั้ง บอร์ดไม่มีทางรู้จัก
ทีมที่ลองแล้วต่อไม่ติดไม่ต้องดีบักโค้ด ให้กลับไปใช้โฮสต์ที่ผู้สอนให้
...
การพิสูจน์อุปกรณ์ต้องใช้ mTLS ที่มีกุญแจส่วนตัวอยู่ในตัวอุปกรณ์จริง ๆ ก่อนเริ่มโค้ด ทุกทีมยังต้องมี `device_id` ของตัวเอง
เพราะบอร์ดออกจากโรงงานด้วยค่าเริ่มต้นเดียวกันหมด ถ้าใช้ซ้ำ broker จะเตะตัวเก่าออกวนไปทั้งห้อง ทั้งที่โค้ดทุกทีมถูก
...
4. ข้อใดถูกเกี่ยวกับพอร์ตและ root CA ของเส้นทาง `tesaiot` เลือกทุกข้อที่ถูก *(เลือกได้หลายข้อ · เป้าหมายข้อ 3)*
   ...
   - ค) MQTTs เข้า CE ที่ทีมติดตั้งเองไม่ผ่าน เพราะ CE สุ่ม root CA ใหม่ทุกการติดตั้ง แต่บอร์ดรู้จักแค่ root ที่คอมไพล์มากับเฟิร์มแวร์
...
- [ ] จดค่าประจำตัวสี่ค่าที่ได้จากผู้สอน (`device_id` · `api_key` · `mqtt_pass` · ชื่อโฮสต์ broker) และนับว่า `device_id` ไม่เกิน 31 ตัวอักษร
...
- ถ้ามีคนได้ `mqtt_pass` ของทีมไป serverTLS ช่วยอะไรได้บ้าง และช่วยอะไรไม่ได้
