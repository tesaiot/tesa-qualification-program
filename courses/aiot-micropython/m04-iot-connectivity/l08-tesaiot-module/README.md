...
สามไฟล์ที่ต้องทำในบทเรียน เปิดตามลำดับนี้ ทั้งชุดราว 29 นาที ทั้งสามรันได้เมื่อทีมมี `device_id` ของตัวเองแล้ว
...
1. `01_config_store.py` (6 นาที) อ่านค่าที่บอร์ดเก็บไว้ทั้ง 19 คีย์ แล้วกรอกกล่องค่าประจำตัวในบันทึกการเรียนจากค่าที่บอร์ดตอบจริง
   ไม่ใช่จากใบที่ผู้สอนแจก สังเกตบรรทัดสีส้มที่บอกว่า `mqtt_pass` ตั้งได้แต่อ่านกลับไม่ได้
...
4. ทีมลองตั้งค่าไปหลายคีย์จนไม่รู้ว่าตอนนี้บอร์ดเหลือค่าอะไรอยู่ ทางที่สไลด์แนะนำคืออะไร *(เลือกหนึ่งข้อ · เป้าหมายข้อ 4)*
   ...
   - ข) เรียก `config_reload()` แล้วค่าของทีมจะกลับมาครบเสมอ
...
- [ ] รัน `06_secure_publish_loop.py` จนตัวนับบนจอเดินพร้อมกราฟของอุปกรณ์ทีมบน dashboard แล้วถ่ายภาพทั้งสองจอ
...
- ทำไมการเขียนลงชิปที่ย้อนกลับไม่ได้ ถึงควรมีกติกาห้ามแตะในห้องเรียน แม้โค้ดจะเรียกได้ปกติ
