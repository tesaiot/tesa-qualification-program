...
แก้ `WIFI_SSID` กับ `WIFI_PASS` ที่หัวไฟล์เป็นวงที่ผู้สอนแจก วงนั้นต้องตั้งรหัสผ่านไว้ เพราะ `connect()` ต่อวงเปิดไม่ได้
และถามผู้สอนไว้ก่อนว่าเกตเวย์ของห้องเป็นเลข `.1` หรือไม่
ทีมละหนึ่งบอร์ด สลับกันพิมพ์ทุกช่วง คนที่ไม่ได้พิมพ์จับเวลาและจดตัวเลขที่เห็นลงบันทึกการเรียน
...
ส่วนไฟล์ `10` กับ `11` ตอบปัญหาที่เจอจริงเมื่อ 14 ส.ค. 2026 ว่ารหัสผ่านของห้องไม่ควรฝังอยู่ในโค้ด ให้พิมพ์บนจอด้วย
...
แล้วได้ภาพที่ไม่เคยเกิดจริง ใช้ `05_status_dict.py` (อ่านครั้งเดียวต่อรอบ) · ไม่รู้ว่าวงไหนคือ AP ของห้อง ใช้ `02_rank_by_rssi.py`
...
- [m04-iot-connectivity/l02-network-status-code/examples/08_softap_fallback.py](../l02-network-status-code/examples/08_softap_fallback.py) — ถ้าในห้องไม่มีวงให้ต่อ บอร์ดปล่อยวงของตัวเองได้
...
ให้ถามเลขจริงจากผู้สอนแล้วแก้บรรทัด `gw = ...`
...
3. หน้าจอของทีมแสดงเกตเวย์ 4 ms แต่อินเทอร์เน็ตไม่ตอบทุกครั้งตลอดสองนาที ทีมควรทำอย่างไร *(เลือกหนึ่งข้อ · เป้าหมายข้อ 2)*
   ...
   - ค) จดผลลงบันทึกการเรียนแล้วทำงานต่อ เพราะปัญหาอยู่ที่ทางออกอินเทอร์เน็ตของห้อง ไม่ใช่บั๊กของเรา
   - ง) เปลี่ยน NET_TEST_IP เป็น "google.com"

   <details><summary>เฉลย</summary>

   **ค** — เกตเวย์ตอบแปลว่าลิงก์ WiFi และเลขเกตเวย์ถูก ปัญหาอยู่หลังเราเตอร์ อาจเป็นทางออกของห้องหรือถูกกรองไว้ ส่วน ping รับเฉพาะเลข IP ใส่ชื่อโฮสต์จะได้ ValueError
