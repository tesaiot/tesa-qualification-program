...
#
# ประโยคสุดท้ายนั้นมีตัวอย่างเป็นเรื่องเป็นราวอยู่ที่ m02-ui-to-hardware/l05-event-loop/examples/03_switch_matches_led.py
...
for i in range(len(LED_IDX)):
    ...
    # ยังไม่ชินกับอาร์กิวเมนต์ของ widget: m02-ui-to-hardware/l05-event-loop/examples/01_first_widgets.py สร้างทีละตัวช้า ๆ
    #   และอธิบายว่าทำไมต้องใส่ x= y= ทุกครั้ง (ถ้าไม่ใส่ จอจะจัดเองบนกริด 200 พิกเซล
    #   แล้วข้อความยาวจะล้นไปทับช่องถัดไป) ส่วน m02-ui-to-hardware/l05-event-loop/examples/06_layout_budget.py คือเรื่องโควตา
...
def set_led(i, on):
    ...
    if on:
        ...
        #   m02-ui-to-hardware/l05-event-loop/examples/03_switch_matches_led.py ให้ปุ่ม ALL OFF ที่ดับไฟจริงแต่ลืมดึงสวิตช์
...
while True:
    ...
    for ev in ui.poll():
        ...
        t = ev['type']
        if t != 'clicked':
            # ตัวควบคุมแต่ละชนิดส่ง type ไม่เหมือนกัน m02-ui-to-hardware/l05-event-loop/examples/02_event_types.py
    ...
    # อยากได้เสียงตอบรับตอนแตะปุ่มด้วย: m02-ui-to-hardware/l05-event-loop/examples/05_sound_feedback.py ต่อ ui.sfx และ ui.tone
