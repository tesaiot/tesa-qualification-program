...
ระดับ **L3** · สถานะ **alpha** (ทดลองใช้) · 6 โมดูล 17 บทเรียน · ประมาณ 30 ชั่วโมง
...
- ตัวอย่างและแบบฝึกทุกไฟล์ของหลักสูตรรันบนคอมพิวเตอร์ได้โดยไม่ต้องมีบอร์ด ต้องมี gcc หรือ clang, make, git และ python3 ส่วนแล็บของแต่ละบทใช้บอร์ด
- การ build เฟิร์มแวร์ใช้ไฟล์ zip ของ release `fw-c-only-v1.10.0` ของ SDK ไม่ใช่ git clone เพราะ git repo ไม่มีไลบรารีที่ build แล้ว (`*.a`) รายละเอียดอยู่ใน[บทเรียน 2.1](m02-build-and-version/l01-toolchain-first-build/README.md)
...
หลักสูตรอยู่ในสถานะ alpha (ทดลองใช้) ทั้ง 17 บทเรียนมีเนื้อหาครบตามโครงเดียวกัน คือ เป้าหมาย ก่อนเริ่ม ดูของจริงก่อน แนวคิด ตัวอย่างสมบูรณ์ ฝึกเติม เฉลย
เช็กความเข้าใจ (ไฟล์ `quiz.yaml` ของแต่ละบท) แล็บ และไปต่อ

- โค้ดตัวอย่างและแบบฝึกทุกไฟล์ตรวจบนคอมพิวเตอร์แล้ว แบบฝึกทุกไฟล์ล้ม test ก่อนเติม และเฉลยทุกไฟล์ผ่าน ยกเว้นสคริปต์ GDB ของบทเรียน 3.1 ที่ตรวจได้เฉพาะเมื่อต่อบอร์ด
- แล็บบนบอร์ดเขียนจากโค้ดและเอกสารของ SDK ที่ commit `ef72c1b` แต่ยังไม่ได้ทดลองสอนบนบอร์ดจริง ถ้าพบขั้นตอนที่ไม่ตรงกับบอร์ด แจ้งได้ตาม [CONTRIBUTING.md](../../CONTRIBUTING.md)
- ลิงก์ไปยัง SDK ทุกลิงก์ตรึงไว้ที่ commit เดียวกัน และตรวจแล้วว่าไฟล์มีอยู่จริง ณ commit นั้น
- ข้อเท็จจริงเรื่องฮาร์ดแวร์และ API ทุกข้ออ้างจากซอร์สของ SDK หรือจาก Infineon mtb-pdl-cat1 release-v3.24.0 ตัวอย่างของ Infineon ใช้วิธีลิงก์ไปยัง repository และ tag ไม่คัดลอกมา
...
- Unity (MIT) ที่ใช้ในโมดูล 6 ผู้เรียน clone เองที่ tag v2.7.0 ไม่ได้รวมไว้ใน repository นี้
