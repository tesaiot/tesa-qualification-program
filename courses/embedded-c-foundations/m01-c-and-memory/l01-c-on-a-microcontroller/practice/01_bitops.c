...
//
// practice/01_bitops.c — ฝึกเติม: ฟังก์ชันตั้งและล้างบิตแบบ read-modify-write
//
// โจทย์: เติม bits_set() และ bits_clear() ให้ test ทุกข้อผ่าน (มีช่องให้เติม 2 จุด มองหา TODO)
// คอมไพล์และรัน:
//     gcc -std=c11 -Wall -Wextra -o bitops 01_bitops.c && ./bitops
// ตอนยังไม่เติม โปรแกรมคอมไพล์ผ่านแต่ test ล้ม นั่นคือสิ่งที่ควรเห็นก่อนเริ่ม
// ลองเองก่อนอย่างน้อย 15 นาที แล้วค่อยเปิด solution/01_bitops.c เทียบ
#include <inttypes.h>
#include <stdint.h>
#include <stdio.h>
...
static void bits_set(volatile uint32_t *reg, uint32_t mask)
{
    (void)reg;
    (void)mask;
    /* TODO: เขียนหนึ่งบรรทัดแบบ read-modify-write ด้วยตัวดำเนินการ OR */
}
...
static void bits_clear(volatile uint32_t *reg, uint32_t mask)
{
    (void)reg;
    (void)mask;
    /* TODO: เขียนหนึ่งบรรทัดด้วย AND กับ mask ที่กลับบิตแล้ว */
}
