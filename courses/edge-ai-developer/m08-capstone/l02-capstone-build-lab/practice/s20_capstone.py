# s20_capstone.py - Capstone: Edge AI Guardian (ฉบับฝึกเติมโค้ด)
# วิธีรัน: 1) เปิดใน BENTO IDE (บอร์ดจริง) หรือ BENTO Emulator ที่ ide.tesaiot.dev
#          2) เติมช่องว่าง (pass / None) ทั้ง 5 จุดตามคำใบ้ `# เติม:` ให้ครบ
#          3) กด "Program to Device" (หรือ Run บน Emulator) แล้วกด Load
#          4) ทำท่า/ส่งเสียงตามโมเดล พอคลาสเป้าหมายชนะติดกันครบเกณฑ์ Guardian จะสั่งการเอง
#
# นี่คือ "โครงตั้งต้น" ของงานจบคอร์ส (capstone) — ไม่ใช่แอปสาธิตชิ้นเดียว แต่เป็น
# แบบร่างของ "ผลิตภัณฑ์ Edge AI" ที่ร้อยหลายเสาเข้าด้วยกันในไฟล์เดียว:
...
# งานของคุณคือเติม 5 บรรทัดที่เป็น "กระดูกสันหลัง" ของทั้งสามเสา ส่วนที่เหลือวางโครงไว้ให้แล้ว
...
lcd.clear()
lcd.console('<h2> Edge AI Guardian - งานจบคอร์ส (ฉบับฝึก)</h2>')


def find_model(keyword):
    """หาโมเดลจากชื่อในทะเบียนของเฟิร์มแวร์ — ไม่เจอก็ใช้ตัวแรกกันแอปพัง
    ถามฮาร์ดแวร์ก่อน (models()) แทน hard-code index เหมือนที่ทำมาตั้งแต่บทเรียน 1.1–1.3"""
    ms = edge_ai.models()
    for m in ms:
...
# baby_cry, Push, cough) ส่วนคลาสแรกมักเป็น idle/silence/unlabelled
    ...
    while True:
        for ev in ui.poll():
            ...
            if h == back_id:
                ...
                try:
                    # เติม: (เสา Apps) สั่งให้ CM55 เริ่มรันโมเดลที่เลือก
        ...
        if running:
            # เติม: (เสา DAQ) อ่านความเร่งดิบ 3 แกนมาเป็นบริบทข้างๆ verdict
            ...
            # เติม: (เสา Apps) อ่านผลอนุมานล่าสุดมาเก็บใน r (dict หรือ None)
            #       -> r = edge_ai.result()
            r = None
            pass
            if r and r['seq'] != last_seq:      # วาด/ตัดสินใจเฉพาะตอนมีผลใหม่ (seq เปลี่ยน)
                ...
                # เติม: (เสา Processing) กรองความมั่นใจให้นิ่งด้วย EMA เก็บผลใน conf_s
                ...
                if streak >= HITS_NEEDED and not fired:
                    # เติม: (เสา Apps) สั่งการจริง -> fire_alert(r['label'], conf_s)
                    pass
                    fired = True
